...
if (isset($_POST['submit'])) {
    ...
    $pried_2 = trim($_POST['pried_2']);
    $subject_2 = trim($_POST['subject_2']);

        $stmt = $conn->prepare("INSERT INTO staff_time_table (day_id,class_id,staff_id,pried_2,subject_2) VALUES (?,?,?,?,?)");
        $stmt->bind_param("sssss",$day_id, $class_id, $staff_id=$staff_id, $pried_2,$subject_2);
        $stmt->execute() or die($stmt->error);
        $id=$stmt->insert_id;
	    header("location: staff-time-table.php?id=$staff_id");
                            ...
                            <tr>
								...
								<div class="form-group">
                                            ...
                                            <select name="day_id" class="form-control" required="required" >
                                                ...
                                                <?php
                                                 $sql2 = "select * from days where states='Active'";
 ...
 <td>
								<div class="form-group">
                                            ...
                                            <select name="class_id" class="form-control" required="required" >
                                                <option value="">-----Select------</option>
                                                <?php
                                                 $sql2 = "select a.*,b.standard,section_name from assign_staff a,class b where a.staff_id=$staff_id and a.class_id=b.id";

                                                $result2 = mysqli_query($conn, $sql2);
                                                while ($row2 = mysqli_fetch_assoc($result2)) {
                                                    ?>
                                                    <option value="<?php echo $row2['class_id']; ?>" ><?php echo $row2['standard']; ?> <?php echo $row2['section_name']; ?></option>
												<?php }?>
                                          </select>
                                        </div>
										</td>
										<td>
                               <div class="form-group">
                                            <label for="pried_2 required"
                                                   class="control-label required">Pried 2</label>
                                            <select name="pried_2" class="form-control" required="required" >
                                                <option value="">-----Select------</option>
                             ...
                             <td>
								<div class="form-group">
                                            <label for="subject_2 required"
                                                   class="control-label required">Subject</label>
                                            <select name="subject_2" class="form-control" required="required" >
                                                <option value="">-----Select------</option>
													<option value="Leisure Time">Leisure Time</option>
													<?php
                                                 //$sql2 = "select * from subject";
                                                 $sql2 = "select a.*,b.sub_name from assign_staff a,subject b where a.staff_id=$staff_id and a.subject_id=b.id";
